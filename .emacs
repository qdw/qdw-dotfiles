...
(defun replace-muffin-regexp-in-region (start end)
  (interactive "*r")
  (save-excursion
    (save-restriction
      (narrow-to-region start end)
      (goto-char (point-min))
      (while (re-search-forward "[:word:]\{5,}" nil t)
             (replace-match     "muffin"        nil nil)))))

;;;;;;;;;;;;;;;
;; Silly stuff
(defun muffin ()
  "Replace all words of five or more characters with the word \"muffin\".
Relies on an external Perl program, because doing this in elisp would be lame"
  (interactive)
  (call-process "muffin" buffer-file-name 't 't))

(defun muffin-region (start end)
  (interactive "*r")
  (replace-regexp-in-region (start
                             end
                             "[:word:]\{5,}"
                             "muffin")))
...
;; (i.e. the one where the cursor does not appear).
;; this is convenient at times, since it saves you from constantly
;; C-x b'ing betwee buffers, and it's unintrusive; no one else uses
;; this key sequence.
...
;; Accept files sent to us via emacsclient.
(server-start nil)
