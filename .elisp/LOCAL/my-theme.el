;; This is a very slight tweak of color-theme-xemacs, which is
;; bundled with the color-theme distro.  The only difference is that I've
;; changed the modeline background to Gray90, so it's lighter than the
;; background in buffers.  That way I can distinguish the modeline
;; visually; with split-window-vertically, I can better see the split between
;; the two buffers.

(eval-when-compile
  (require 'color-theme))

(defun color-theme-xemacs-tweaked ()
  "XEmacs standard colors, but with a darker modeline (useful for split-window-vertically)
...
XEmacs color theme."
  ...
  (color-theme-install
   '(color-theme-xemacs
    ...
    (modeline ((t (:background "Gray90"))))
    (modeline-buffer-id ((t (:background "Gray90" :foreground "blue4"))))
    (modeline-mousable ((t (:background "Gray90" :foreground "firebrick"))))
    (modeline-mousable-minor-mode ((t (:background "Gray90" :foreground "green4"))))
    ...
    (widget-inactive-face ((t (:foreground "Gray80"))))
